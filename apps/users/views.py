    ...
    def authenticate(self, username=None, password=None, **kwargs):
        try:
            user = UserProfile.objects.get(
                Q(username=username) | Q(email=username))  # 输入账号或邮箱进行并集查询
...
class ActiveUserView(View):  # 用户邮箱激活
...
class RegisterView(View):  # 用户注册
    ...
    def post(self, request):
        ...
        if register_form.is_valid():  # 表单验证不为空
            ...
            user_profile.save()  # 将验证码保存到数据库中

            # 写入欢迎注册消息
            user_message = UserMessage()
            user_message.user = user_profile.id
            user_message.message = "欢迎注册菁航教育网"
            user_message.save()
...
class LoginView(View):
    ...
    def post(self, request):
        ...
        if login_form.is_valid():  # 表单验证不为空
            ...
            if user is not None:
                if user.is_active:
                    ...
                    return render(request, 'login.html', {'msg': '用户未激活，请查看注册邮箱进行激活'})
...
class SendEmailCodeView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        if UserProfile.objects.filter(email=email):
            return HttpResponse(
                '{"email":"邮箱已经存在"}', content_type='application/json')
...
class IndexView(View):
    ...
    def get(self, request):
        ...
        course_orgs = CourseOrg.objects.all()[:8]
        return render(
            request, 'index.html', {
